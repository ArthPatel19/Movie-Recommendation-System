...
def get_local_idx_by_title(title: str) -> int:
    ...
    # Try close title matches when the exact TMDB title does not exist in the local dataset.
    keys = list(TITLE_TO_IDX.keys())
    close = difflib.get_close_matches(key, keys, n=1, cutoff=0.75)
    if close:
        return int(TITLE_TO_IDX[close[0]])

    # Fall back to substring matching for titles like "Matrix" vs "The Matrix".
    for candidate in keys:
        if key in candidate or candidate in key:
            return int(TITLE_TO_IDX[candidate])
